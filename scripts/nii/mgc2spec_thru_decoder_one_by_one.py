...
bottleneck_size = model.encode_layers[-1].dim
...
for mgc in mgc_files:
    ...
    with open(mgc) as fb:
        ...
        assert mgc_arr.shape[0] % bottleneck_size == 0, \
            'clip.shape[0]:%s, bottleneck_size:%s'%(mgc_arr.shape[0], bottleneck_size)
        mgc_npy = mgc_arr.reshape((mgc_arr.shape[0]/bottleneck_size, bottleneck_size))
        ...
        #         print "buffer", proc.buffer
        #         print "scale_range", proc.scale_range
        ...
        if args.preprocessor:
            ...
            if proc.__class__.__name__ == "Scale":
                ...
                print "buffer", proc.buffer
                print "scale_range", proc.scale_range
            print 'inverting: ' + args.preprocessor
            output = proc.invert(output)
